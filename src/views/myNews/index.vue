        ...
        <template slot="firstColumn">
          <el-table-column label="标题"
            ...
            <template slot-scope="scope">
              <a type="text"
                 :style="{color:'#409EFF',fontWeight:scope.row.user_id?'normal':'bold'}"
                 @click="seeView(scope.row)">
                {{scope.row.notice_title}}</a>
            </template>
          </el-table-column>
...
export default {
  ...
  computed: {
    tableHeight: function() {
      return this.$store.state.app.dialogHeight / 0.75 * 0.85 - 310
    }
  },
  created() {
    this.getOption()
