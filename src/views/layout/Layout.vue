...
export default {
  ...
  created() {
    ...
    window.setInterval(_ => {
      ...
      const overTimeCache = []
      for (const iterator of this.$store.state.app.cacheData) {
        if (currentTime - iterator.time > interval) {
          overTimeCache.push(iterator)
        }
      }
      if (overTimeCache.length > 0) {
        this.$store.dispatch('deleteCacheData', overTimeCache)
      }
    }, interval)
  },
  data() {
