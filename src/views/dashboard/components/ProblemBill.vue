  ...
  <div class="app-container">
          ...
          <tr class="tr-height">
            ...
            <td>
              <el-select :disabled="disabled"
                         v-model="formData.事项分类"
                         clearable
                         filterable
                         placeholder=""
                         style="width:100%;">
                <el-option label="数据质量问题"
                           value="0"></el-option>
                <el-option label="报送问题"
                           value="1"></el-option>
                <el-option label="数据格式问题"
                           value="2"></el-option>
              </el-select>
            </td>
        ...
        <div class="row-option">
          <el-upload class="upload-demo"
            ...
            <el-button slot="trigger"
                       ...
                       @click="uploadAttachment">上传附件</el-button>
            <el-button icon="el-icon-download"
                       @click="batchDownload">批量下载</el-button>
            <a @click="attachmentTableToggle=false"
               v-if="attachmentTableToggle">
              <svg-icon icon-class="up" />&nbsp;收起</a>
            <a @click="attachmentTableToggle=true"
               v-else>
              <svg-icon icon-class="down" />&nbsp;展开</a>
          </el-upload>
        </div>
      </div>
      <basic-table v-show="attachmentTableToggle"
                   noHeight
                   ref="attachmentTable"
                   :tableOption="attachmentConfigData"
                   :pagenation="false"
                   :rowCheck="true"
                   :rowNum="false">
          ...
          <el-table-column label="操作"
            ...
            <template slot-scope="scope">
              <el-button type="primary"
                         plain
                         size="mini"
                         @click="attchDownload(scope.row)">下载</el-button>
              <el-button type="danger"
                         plain
                         size="mini"
                         @click="attchDelete(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </template>
      </basic-table>
    ...
    <div class="base-row">
      <div class="row-botton clearfix">
        ...
        <div class="row-option">
          <a @click="handleRecordTableToggle=false"
             v-if="handleRecordTableToggle">
            <svg-icon icon-class="up" />&nbsp;收起</a>
          <a @click="handleRecordTableToggle=true"
             v-else>
            <svg-icon icon-class="down" />&nbsp;展开</a>
        </div>
      </div>
      <basic-table v-show="handleRecordTableToggle"
                   noHeight
                   ref="handleRecordTable"
                   :tableOption="handleRecordConfigData"
                   :pagenation="false"
                   :rowNum="true">
      </basic-table>
    </div>
    <div class="base-row">
      ...
      <el-form ref="itemForm"
        ...
        <table border="0"
          ...
          <tr class="tr-height">
            <td class="td-align"
                style="width:15%;"><span class="input-label">处理人</span></td>
            <td>
              <el-select v-model="itemFormData.处理人"
                         clearable
                         filterable>
                <el-option label="处理人"
                           value="1"></el-option>
          ...
          <tr class="tr-height">
            <td class="td-align"
                style="width:15%;"><span class="input-label">处理意见</span></td>
            <td>
              <el-input v-model="itemFormData.处理意见"
                        type="textarea"
                        style="width:500px;"
                        clearable></el-input>
            </td>
          ...
          <tr class="tr-height">
            <td class="td-align"
                style="width:15%;"><span class="input-label">通知方式</span></td>
            <td>
...
const attachmentUrl = '/east-current/datamgr/getAttachments'
const handleRecordUrl = '/east-current/datamgr/getHandleRecords'
export default {
