 ...
 * @Last Modified by: lk
 * @Last Modified time: 2020-05-07 09:17:51
 */
...
const service = axios.create({
  // baseURL: store.state.app.baseUrl ? store.state.app.baseUrl : process.env.BASE_API, // api的base_url
  baseURL:
    process.env.NODE_ENV === 'development'
      ? process.env.BASE_API
      : window.g.baseUrl, // api的base_url
...
service.interceptors.request.use(
  config => {
    ...
    const curretPath = temp[temp.length - 1]
    localStorage.setItem('requestFromPath', curretPath)
    NProgress.start()
      ...
      if (res.code !== 1) {
        ...
        if (res.data) {
          ...
          if (permissionData && permissionData.length > 0) {
            store.dispatch(
              'changeErrorList',
              JSON.stringify(permissionData) + '$' + new Date().getTime()
            )
          }
        }
        NProgress.done()
